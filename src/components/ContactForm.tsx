...
const isRateLimited = () => {
  try {
    ...
    const now = Date.now();
    const recent = stored.filter((ts: number) => now - ts < RATE_LIMIT_WINDOW_MS);
...
const ContactForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if ((form.elements as any).website?.value) return;

    const newErrors = validate();
    if (Object.keys(newErrors).length > 0) {
      setErrors(newErrors);
      const firstErrorField = Object.keys(newErrors)[0];
      const refs: Record<string, React.RefObject<HTMLElement | null>> = { name: nameRef, email: emailRef, message: messageRef };
